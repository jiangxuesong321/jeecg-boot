...
//				emailHandle.sendTemplateMail("招标通知书",context,emails,null);
//			}else{
    ...
    public void submitTemplate(BiddingTemplatePage page) {
        ...
        for (BiddingSupplier bs : detaiList) {
            ...
            for (BiddingRecordToProfessionals bps : bpList) {
                if (bps.getBsId().equals(bs.getId())) {
                    BigDecimal itemScore = bps.getItemScore() == null ? BigDecimal.ZERO : bps.getItemScore();
                    BiddingProfessionals entity = map.get(bps.getBpsId());
                    BigDecimal weight = new BigDecimal(entity.getWeight());
                    itemScore = itemScore.multiply(weight.divide(new BigDecimal(100))).setScale(2, BigDecimal.ROUND_HALF_UP);
                    score = score.add(itemScore);
                }
            }
            bs.setItemTotal(score);
        }
...
//                        EmailSendMsgHandle emailHandle = new EmailSendMsgHandle();
//
////						emailHandle.sendTemplateMail("中标通知书",context,emails,null);
//                    } else {
//                        log.error("招标中标通知书,该供应商没有设置邮箱地址");
//                    }
////					if("1".equals(main.getIsNotice())){
